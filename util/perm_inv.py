...
class einpool(nn.Module):
    ...
    def forward(self,X,mask=None,to_cpu=False):
        ...
        for s in self.eqs:
            #print(s)
            h_i=einsum.einsum(s,X)
            if to_cpu:
                h_i=h_i.cpu()
            
            n_i=einsum.einsum(s,mask)
            if to_cpu:
                n_i=n_i.cpu()
...
class einpool_multihead(nn.Module):
    ...
    def forward(self,X,mask=None,to_cpu=False):
        if not X.shape[-1]==self.nheads():
            X=X.view(*X.shape[:-1],-1,self.nheads())
...
class einnet(nn.Module):
    def __init__(self,ninput,nh0,nh,nstacks=2,nlayers=2,form='X_Bab',order=5,deps=[],multihead=False):
        super().__init__()
        
        if multihead:
            self.pool=einpool_multihead(form=form,order=order,deps=deps,equivariant=True)
            n=len(self.pool.eqs)
            self.nheads=self.pool.nheads()
        else:
            self.pool=einpool(form=form,order=order,deps=deps,equivariant=True)
            n=len(self.pool.eqs)
            self.nheads=1
        
        self.t=nn.ModuleList()
        self.t0=MLP(ninput,nh,self.nheads*nh0,nlayers)
        
        for i in range(nstacks):
            self.t.append(MLP(n*nh0,nh,self.nheads*nh0,nlayers))
    ...
    def forward(self,X,mask=None):
        ...
        for layer in self.t:
            h=h*mask
            skip=h
            h=self.pool(torch.sin(h),mask).view(*h.shape[:-1],-1)
            h=layer(h)
            h=h*mask
            h+=skip
