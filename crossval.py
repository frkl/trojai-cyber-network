    ...
    for split_id,split in enumerate(crossval_splits):
        ...
        for iter in range(params.epochs):
            ...
            with torch.no_grad():
                ...
                tracker.add(loss_val=loss_val)
                
                if loss_val<best_loss:
                    best_loss=loss_val
                    best_net=copy.deepcopy(net)
            
            print('%d/%d,%s, time %.2f'%(iter,params.epochs,tracker.str(),time.time()-t0),end='\r')
...
def crossval_hyper(dataset,params):
    ...
    hp_config.add('qloguniform','batch',low=2,high=8);
...
if __name__ == "__main__":
    ...
    if params.data.endswith('.pt'):
        try:
            ...
            dataset=db.Table(torch.load(params.data))
